...
def test_ensure_project_paths_creates_builder_directories(tmp_path, monkeypatch):
    ...
    assert (tmp_path / "vault/kb/gtfobins/tools").is_dir()
    assert (tmp_path / "vault/kb/payloads").is_dir()
    assert (tmp_path / "vault/kb/internal").is_dir()
    assert (tmp_path / "vault/kb/hacktricks").is_dir()
