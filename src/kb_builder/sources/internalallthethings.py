...
from ..models import ExternalReference, InternalTopic
from ..naming import slugify
from ..paths import resolve_repo_path


class InternalAllTheThingsSource:
    """
    Loader/parser for InternalAllTheThings Markdown topics.

    InternalAllTheThings is treated as internal pentest/reference Markdown,
    preserving each source Markdown page as one generated note.
    """
    ...
    def load(self) -> list[dict[str, Any]]:
        ...
        if not local_path:
            raise ValueError(
                "InternalAllTheThings source requires sources.internalallthethings.local_path"
            )

        path = resolve_repo_path(local_path, "sources.internalallthethings.local_path")
        files = self._markdown_files(path)
        self.logger.info(f"Loading InternalAllTheThings Markdown files: {len(files)}")
    ...
    def parse(self, records: list[dict[str, Any]]) -> list[InternalTopic]:
        topics: list[InternalTopic] = []
        ...
        self.logger.info(f"Parsed InternalAllTheThings topics: {len(topics)}")
        return topics
    ...
    def _markdown_files(self, path: Path) -> list[Path]:
        ...
        if not path.is_dir():
            raise FileNotFoundError(f"InternalAllTheThings local_path not found: {path}")
    ...
    def _parse_topic(self, record: dict[str, Any]) -> InternalTopic | None:
        ...
        return InternalTopic(
            ...
            raw=record,
            external_references=[
                ExternalReference(source_name="internalallthethings", url=source_url)
            ],
            category=category,
    ...
    def _description(self, body: str, title: str) -> str:
        ...
        return f"InternalAllTheThings topic for {title}."

    def _source_url(self, relative_path: str) -> str:
        encoded = "/".join(part.replace(" ", "%20") for part in relative_path.split("/"))
        return f"https://github.com/swisskyrepo/InternalAllTheThings/blob/main/docs/{encoded}"

    def _tags(self, category: str) -> list[str]:
        return sorted(
            {
                "internalallthethings",
                "internalallthethings/topic",
                f"internalallthethings/category/{slugify(category)}",
            }
        )
