...
from ..models import ExternalReference, HackTricksTopic
from ..naming import slugify
from ..paths import resolve_repo_path


class HackTricksSource:
    """
    Loader/parser for HackTricks Markdown topics.

    HackTricks is treated as topic/reference Markdown, preserving each source
    Markdown page as one generated note.
    """

    TRAINING_BANNER_PATTERN = re.compile(
        r"^\s*\{\{#include\s+[^}]*banners/hacktricks-training\.md\s*\}\}\s*$\n?",
        flags=re.MULTILINE,
    )
    ...
    def load(self) -> list[dict[str, Any]]:
        ...
        if not local_path:
            raise ValueError("HackTricks source requires sources.hacktricks.local_path")

        path = resolve_repo_path(local_path, "sources.hacktricks.local_path")
        files = self._markdown_files(path)
        self.logger.info(f"Loading HackTricks Markdown files: {len(files)}")
    ...
    def parse(self, records: list[dict[str, Any]]) -> list[HackTricksTopic]:
        topics: list[HackTricksTopic] = []
        ...
        self.logger.info(f"Parsed HackTricks topics: {len(topics)}")
        return topics
    ...
    def _markdown_files(self, path: Path) -> list[Path]:
        ...
        if not path.is_dir():
            raise FileNotFoundError(f"HackTricks local_path not found: {path}")
    ...
    def _skip_path(self, parts: tuple[str, ...]) -> bool:
        ...
        if len(parts) == 1:
            return True
        if parts[0].lower() == "banners":
            return True
        return any(part.startswith((".", "_")) for part in parts)

    def _parse_topic(self, record: dict[str, Any]) -> HackTricksTopic | None:
        ...
        topic_id = f"hacktricks-{slugify(relative_path.removesuffix('.md'))}"
        ...
        output_path = f"kb/hacktricks/{output_dir}/{page_slug}.md" if output_dir else f"kb/hacktricks/{page_slug}.md"
        source_url = self._source_url(relative_path)

        return HackTricksTopic(
            id=topic_id,
            source="hacktricks",
            ...
            raw=record,
            external_references=[
                ExternalReference(source_name="hacktricks", url=source_url)
            ],
            category=category,
    ...
    def _body_content(self, body: str) -> str:
        ...
        content = self._remove_training_banner_include(content)
        return self._remove_summary_section(content)

    def _remove_training_banner_include(self, body: str) -> str:
        return self.TRAINING_BANNER_PATTERN.sub("", body).strip()
    ...
    def _description(self, body: str, title: str) -> str:
        ...
        return f"HackTricks topic for {title}."

    def _source_url(self, relative_path: str) -> str:
        encoded = "/".join(part.replace(" ", "%20") for part in relative_path.split("/"))
        return f"https://github.com/HackTricks-wiki/hacktricks/blob/master/src/{encoded}"
        ...
        return sorted(
            {
                "hacktricks",
                "hacktricks/topic",
                f"hacktricks/category/{slugify(category)}",
            }
        )
