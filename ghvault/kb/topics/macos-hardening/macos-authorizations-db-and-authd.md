...
The database located in /var/db/auth.db is database used to store permissions to perform sensitive operations. These operations are performed completely in user space and are usually used by XPC services which need to check if the calling c
...
[source record](../../sources/hacktricks/macos-authorizations-db-and-authd.md)
