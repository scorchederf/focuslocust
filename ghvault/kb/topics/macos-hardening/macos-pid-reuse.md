...
When a macOS XPC service is checking the called process based on the PID and not on the audit token, it's vulnerable to PID reuse attack. This attack is based on a race condition where an exploit is going to send messages to the XPC service
...
[source record](../../sources/hacktricks/macos-pid-reuse.md)
