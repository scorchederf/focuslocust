...
Notepad++ will autoload every plugin DLL found under its plugins subfolders on launch. Dropping a malicious plugin into any writable Notepad++ installation gives code execution inside notepad++.exe every time the editor starts, which can be
...
[source record](../../sources/hacktricks/notepad-plugin-autoload-persistence-and-execution.md)
