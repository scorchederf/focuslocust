...
This is a quick lab to familiarize with an Active Directory Certificate Services (ADCS) + PetitPotam + NLTM Relay technique that allows attackers, given ADCS is misconfigured (which it is by default), to effectively escalate privileges from
...
![PetitPotam: NTLM Relay to AD CS flow diagram](<../../_assets/image (1084) (1) (1).png>)
...
![Installing impacket and switching to the adcs attack branch](<../../_assets/image (1024).png>)
...
![Installing python3 virtual environment](<../../_assets/image (1027).png>)
...
![Initiating and activating the impacket virtual environment](<../../_assets/image (1028).png>)
...
![NTLM relay is ready and waiting for incoming authentications](<../../_assets/image (1032).png>)
...
![DC01 is coerced to authenticate to 10.0.0.5. DC01$ certificate is retrieved from CA01](<../../_assets/image (1033).png>)
...
![TGT for DC01$ retrieved and injected into the current logon session](<../../_assets/image (1034).png>)

`klist` confirms we now have a TGT for `DC01$` in the current logon session:

![TGT for DC01$ in memory](<../../_assets/image (1035).png>)
...
![DCSync pulls NTLM hash of krbtgt](<../../_assets/image (1036).png>)
...
![LDAP relay succeeds, delegation rights setup](<../../_assets/image (1043).png>)

Below screenshot shows that the `QUAIIVEE` computer account has been indeed created and `WS01$` has some privileges to it:

![Computer AD object created as part of RBCD attack](<../../_assets/image (1044).png>)
...
![WS01 has been configured for RBCD attack](<../../_assets/image (1065).png>)
...
![Rubeus calculates the RC4 hash - 3F55290748348504327CDA267FCCA190](<../../_assets/image (1048).png>)
...
![s4u successfully retrieves appropriate TGT and TGS](<../../_assets/image (1049).png>)
...
![C$ share being listed on WS01 from CA01](<../../_assets/image (1051).png>)

### WebClient Service

For the above attack to work, the target system `WS01` has to have the `WebClient` service running:

![WebClient service running on WS01](<../../_assets/image (1053).png>)
...
![Forcing the WebClient service to run](<../../_assets/image (1055).png>)
...
![Regular users can add up to 10 machine accounts in the domain by default](<../../_assets/image (1064).png>)
...
![Password hash calculations](<../../_assets/image (1056).png>)
...
![TGS for administrator@offense.local issued to access cifs/ws01.offense.local](<../../_assets/image (1057).png>)
...
![TGS base64 decoded and saved to a kirbi file](<../../_assets/image (1062).png>)
...
![Converting .kirbi to .ccache](<../../_assets/image (1059).png>)
...
![KRB5CCNAME variable exported](<../../_assets/image (1060).png>)
...
![Code execution on WS01 as offense\administrator](<../../_assets/rbcd-local-privilege-escalation.gif>)

Privileged code execution on `WS01` can also be achieved using impacket's psexec:

![psexec executes code on ws01 from 10.0.0.5 with SYSTEM privileges](<../../_assets/image (1063).png>)
**Note**
...
[source record](../../sources/redteamingtactics/adcs-petitpotam-ntlm-relay-obtaining-krbtgt-hash-with-domain-controller-machine-certificate.md)
