...
Delivery receipts are mandatory in modern end-to-end encrypted (E2EE) messengers because clients need to know when a ciphertext was decrypted so they can discard ratcheting state and ephemeral keys. The server forwards opaque blobs, so devi
...
[source record](../../sources/hacktricks/delivery-receipt-side-channel-attacks-in-e2ee-messengers.md)
