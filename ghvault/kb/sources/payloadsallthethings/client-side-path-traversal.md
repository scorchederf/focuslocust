...
- [Client Side Path Traversal](../../topics/client-side-path-traversal/client-side-path-traversal.md)

## Extracted Fields

| Field | Value |
| --- | --- |
| id | patt-client-side-path-traversal-readme |
| name | Client Side Path Traversal |
| type | payload-topic |
| source | payloadsallthethings |
| url | https://github.com/swisskyrepo/PayloadsAllTheThings/blob/master/Client%20Side%20Path%20Traversal/README.md |
...
_body: "# Client Side Path Traversal\n\n> Client-Side Path Traversal (CSPT), sometimes also referred to as \"On-site Request\
  ...
  * [References](#references)\n\n## Tools\n\n* [doyensec/CSPTBurpExtension](https://github.com/doyensec/CSPTBurpExtension)\
  ...
  \ CSPT to XSS\n\n![cspt-query-param](https://matanber.com/images/blog/cspt-query-param.png)\n\nA post-serving page calls\
  \ the fetch function, sending a request to a URL with attacker-controlled input which is not properly encoded in its path,\
  \ allowing the attacker to inject `../` sequences to the path and make the request get sent to an arbitrary endpoint. This\
  \ behavior is referred to as a CSPT vulnerability.\n\n**Example**:\n\n* The page `https://example.com/static/cms/news.html`\
  \ takes a `newsitemid` as parameter\n* Then fetch the content of `https://example.com/newitems/<newsitemid>`\n* A text injection\
  \ was also discovered in `https://example.com/pricing/default.js` via the `cb` parameter\n* Final payload is `https://example.com/static/cms/news.html?newsitemid=../pricing/default.js?cb=alert(document.domain)//`\n\
  \n### CSPT to CSRF\n\nA CSPT is redirecting legitimate HTTP requests, allowing the front end to add necessary tokens for\
  \ API calls, such as authentication or CSRF tokens. This capability can potentially be exploited to circumvent existing\
  \ CSRF protection measures.\n\n|                                             | CSRF               | CSPT2CSRF          |\n\
  | ------------------------------------------- | -----------------  | ------------------ |\n| POST CSRF ?               \
  \                  | :white_check_mark: | :white_check_mark: |\n| Can control the body ?                      | :white_check_mark:\
  \ | :x:                |\n| Can work with anti-CSRF token ?             | :x:                | :white_check_mark: |\n| Can\
  \ work with Samesite=Lax ?                | :x:                | :white_check_mark: |\n| GET / PATCH / PUT / DELETE CSRF\
  \ ?           | :x:                | :white_check_mark: |\n| 1-click CSRF ?                              | :x:         \
  ...
  * CVE-2023-6458: CSPT2CSRF with a GET sink in Mattermost\n* [Client Side Path Manipulation - erasec.be](https://www.erasec.be/blog/client-side-path-manipulation/):\
  \ CSPT2CSRF `https://example.com/signup/invite?email=foo%40bar.com&inviteCode=123456789/../../../cards/123e4567-e89b-42d3-a456-556642440000/cancel?a=`\n\
  * [CVE-2023-5123 : CSPT2CSRF in Grafana’s JSON API Plugin](https://medium.com/@maxime.escourbiac/grafana-cve-2023-5123-write-up-74e1be7ef652)\n\
  \n## Labs\n\n* [doyensec/CSPTPlayground](https://github.com/doyensec/CSPTPlayground) - CSPTPlayground is an open-source\
  \ playground to find and exploit Client-Side Path Traversal (CSPT).\n* [Root Me - CSPT - The Ruler](https://www.root-me.org/en/Challenges/Web-Client/CSPT-The-Ruler)\n\
  \n## References\n\n* [Exploiting Client-Side Path Traversal to Perform Cross-Site Request Forgery - Introducing CSPT2CSRF\
  \ - Maxence Schmitt - July 2, 2024](https://web.archive.org/web/20260222183040/https://blog.doyensec.com/2024/07/02/cspt2csrf.html)\n\
  ...
  * [Exploiting Client-Side Path Traversal - CSRF is Dead, Long Live CSRF - OWASP Global AppSec 2024 - Maxence Schmitt - June\
  \ 24, 2024](https://web.archive.org/web/20250521192653/https://www.doyensec.com/resources/Doyensec_CSPT2CSRF_OWASP_Appsec_Lisbon.pdf)\n\
  * [Leaking Jupyter instance auth token chaining CVE-2023-39968, CVE-2024-22421 and a chromium bug - Davwwwx - August 30,\
  \ 2023](https://web.archive.org/web/20240703155707/https://blog.xss.am/2023/08/cve-2023-39968-jupyter-token-leak/)\n* [On-site\
  \ request forgery - Dafydd Stuttard - May 3, 2007](https://web.archive.org/web/20260212042947/https://portswigger.net/blog/on-site-request-forgery)\n\
...
_relative_path: Client Side Path Traversal/README.md
_source_path: /home/adams/scorchederf/focuslocust/.cache/payloadsallthethings/Client Side Path Traversal/README.md
```
