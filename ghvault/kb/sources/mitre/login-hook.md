...
- [Login Hook](../../attack/techniques/T1037.002-login-hook.md)
...
description: "Adversaries may use a Login Hook to establish persistence executed upon user logon. A login hook is a plist\
  ...
  \ file, using the <code>LoginHook</code> or <code>LogoutHook</code> key-value pair. The malicious script is executed upon\
  \ the next user login. If a login hook already exists, adversaries can add additional commands to an existing login hook.\
  \ There can be only one login and logout hook on a system at a time.(Citation: S1 macOs Persistence)(Citation: Wardle Persistence\
